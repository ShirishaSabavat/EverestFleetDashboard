...
import { onFetchAllTeamsApi, onFetchWeeklyReportApi, onFetchWeeklyReportUberKmApi } from 'services/axios';
...
const weeklySumReport = ({
  ...
  const [weeklyReportUberKm = [], setWeeklyReportUberKm] = useState();
  ...
  const selectedTeamFunction = (array, data1, data2) => {
    ...
    if (onSelected === false) {
      setSelected({
        ...
        table: <DayWiseTable weeklyReport={data1} weeklyReportUberKm={data2} />,
      });
      setSelectedTable(<DayWiseTable
        weeklyReport={data1}
        weeklyReportUberKm={data2}
      />);
    } else {
      setSelectedTable(
        <DayWiseTable weeklyReport={data1} weeklyReportUberKm={data2} />,
      );
    }
  };
  ...
  useEffect(async () => {
    ...
    hisaabPromises.push(await onFetchWeeklyReportApi(selectedTeam, calenderDate));
    hisaabPromises.push(await onFetchWeeklyReportUberKmApi(selectedTeam, calenderDate));
    ...
    try {
      ...
      setWeeklyReport(response[0][1]?.data);
      setWeeklyReportUberKm(response[0][2]?.data);
  ...
  const menuObject = filteredMenuObject.map((item) => ({
    ...
    table: <DayWiseTable weeklyReport={weeklyReport} weeklyReportUberKm={weeklyReportUberKm} />,
  }));
