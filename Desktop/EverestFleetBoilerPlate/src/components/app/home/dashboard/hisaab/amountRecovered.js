...
// import { Link } from 'react-router-dom';
// import DriverHisaab from '../driverHisaab/index';
// import SeeMorePage from './seeMorePage';

const AmountRecovered = (props) => {
  const {
    // hisaabDriverData = {},
    hisaabAmountRecovered = [],
    // hisaabData = [],
    onSearchDriver,
    hisaabARValue = '',
  } = props;
  ...
  const hisaabAmountRecoveredTemp = hisaabAmountRecovered
    .filter(({
      driver_name: name,
      mobile,
  ...
  const data = hisaabAmountRecoveredTemp?.map((item, index) => ({
    key: index,
  ...
  return (
    <div className="row mt-2">
      <div className="col-12 d-flex flex-row justify-content-between px-4 mb-3">
        <h6 className="text-[#333333] text-md font-bold">Drivers with Amount Recovered</h6>
        <Input
          size="medium"
          // onSearch={onSearch}
      ...
      <div className="col-12">
        ...
        {/* <table className="text-[#333333] text-center text-xs w-100">
          ...
          <tbody>
            {Array.isArray(hisaabAmountRecoveredTemp) && hisaabAmountRecoveredTemp.length > 0
              ? hisaabAmountRecoveredTemp.sort((a, b) => (
...
export default AmountRecovered;
