...
const seeMorePage = (props) => {
  ...
  const { hisaabDriverData = {}, hisaabData = [], onSearchDriver } = props;
  // const [searchLoading, setSearchLoading] = useState(false);
  // const onSearch = () => (value) => (value);
  // global search input on enter
  const globalSearch = (value) => {
    // setTimeout(() => setSearchLoading(false), 500);
    // const enterValueID = Number(value);
    // setDriverID({ enterValue: enterValueID });
    ...
    <div div className="row p-3">
      <div className="col-12 p-0">
        <Input
          ...
          style={{ width: '320px' }}
          className="mt-3 mb-5 bg-[#F1F0F1] text-[#333333]"
      ...
      <div className="col-12 border-solid rounded-md bg-[#FFFFFF] shadow-sm shadow-#0000000F">
        <div className="row p-3">
          <div className="col-1 p-0">
            <img className=" border-solid rounded-full h-100 ml-5 mr-4" width="50" src="https://api.time.com/wp-content/uploads/2017/12/joey-degrandis-hsam-memory.jpg" alt="rupee2" />
          </div>
          ...
          <div className="col-9">
            <div className="d-flex flex-row mb-4">
              <div className="h-9 px-4 text-center border-solid border-r-2 border-[#86A1A4]">
                <h6 className="text-[#FF0000] text-sm font-semibold m-0">
                  {typeof hisaabDriverData?.ledger_balance !== 'number' ? 0
                    : roundNumber(hisaabDriverData?.ledger_balance).toFixed(1)}
                </h6>
              ...
              <div className="h-9 px-4 text-center">
                <h6 className="text-[#FF0000] text-sm font-semibold m-0">
                  {typeof hisaabData?.outstanding !== 'number' ? 0
                    : roundNumber(hisaabData?.outstanding).toFixed(1)}
                </h6>
